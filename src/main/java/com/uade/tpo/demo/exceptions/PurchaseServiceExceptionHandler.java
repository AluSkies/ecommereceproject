...
public class PurchaseServiceExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleUnauthorizedUserAccess(
        UnauthorizedUserAccessException ex, HttpServletRequest request) {
    ...
    public ResponseEntity<ErrorResponse> handleInvalidUserOperation(
        InvalidUserOperationException ex, HttpServletRequest request) {
        log.warn("Operación inválida de usuario en {}: {}", request.getRequestURI(), ex.getMessage());
        ErrorResponse error = ErrorResponse.of(
            "OPERACION_USUARIO_INVALIDA",
            ex.getMessage(),
            request.getRequestURI()
        );
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(error);
