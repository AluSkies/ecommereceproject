...
export function GestionCategorias() {
  ...
  const handleCreate = async (e) => {
    ...
    setCreating(true)
    try {
      // POST /api/v1/categories — el backend exige id numérico + description.
      await apiPost('/categories', { id: Number(newId), description: newDescription })
      setNewId('')
      setNewDescription('')
      fetchCategories()
    } catch (err) {
      setActionError(err instanceof ApiError ? err.message : 'No se pudo crear la categoría.')
    } finally {
      setCreating(false)
    }
  }
  ...
  const handleSaveEdit = async () => {
    setActionError(null)
    try {
      await apiPut(`/categories/${editing.id}`, { id: editing.id, description: editing.description })
      setEditing(null)
      fetchCategories()
    } catch (err) {
      setActionError(err instanceof ApiError ? err.message : 'No se pudo actualizar la categoría.')
    }
  }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setActionError(err instanceof ApiError ? err.message : 'No se pudo eliminar la categoría.')
    }
  }
