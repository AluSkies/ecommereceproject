...
import { fetchCatalogData, clearCatalogCache as dispatchClearCatalog } from '@/redux/productsSlice'

function useCatalogData() {
  const dispatch = useDispatch()
  const { watches, loading, error } = useSelector((state) => state.products)
  const { categories } = useSelector((state) => state.categories)

  useEffect(() => {
    // If not loaded and not loading, fetch catalog data
    if (watches.length === 0 && !loading && !error) {
      dispatch(fetchCatalogData())
    }
  }, [dispatch, watches.length, loading, error])

  const data = useMemo(() => ({ watches, categories }), [watches, categories])
...
export function clearCatalogCache() {
  store.dispatch(dispatchClearCatalog())
}
